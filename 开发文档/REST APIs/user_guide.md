...
| X-NAS-DEVICEID  | String | 否(空字符串) | 设备ID |
| X-NAS-VERSION  | String | 否(1) | NAS接口版本号 |
...
```java
appid, appsecret服务器提供
bodysign=md5(body)
sign=sha256(appId+timestamp+bodysign+appsecret))
签名算法说明：将appId+timestamp+bodysign+appsecret拼接成一个字符串，然后用ssha256计算
```
